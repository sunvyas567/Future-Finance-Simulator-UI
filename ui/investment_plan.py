...
# =========================================================
def render_investment_plan_mobile_old_working_redesigned(user_data: dict, user: dict):
    is_guest = user.get("is_guest", False)
    is_premium = user.get("is_premium", False)
    country = user_data.get("country", "IN")
    currency = get_currency(user_data)
...
<div style="background-color: #1e1e1e; padding: 20px; border-radius: 12px; border-left: 5px solid #6366f1; margin-bottom: 25px;">
    ...
    with ui_section("Passive Income", "💸"):
        ...
        for i, key in enumerate(visible_sources):
            ...
            # Use keys exclusively to prevent state fighting
            value_key = f"income_{active}_{key}_value"
            ...
            with col:
                with st.container(border=True):
                    ...
                    st.number_input(
                        label_text, min_value=0.0, step=1000.0,
                        disabled=not editable, key=value_key, label_visibility="collapsed"
                    )
                    scenario["income_sources"][key] = st.session_state[value_key]
    ...
    with ui_section("Living Expenses", "💳"):
        ...
        wd_key = f"withdrawal_{active}"
        if wd_key not in st.session_state:
            st.session_state[wd_key] = float(scenario["withdrawal"]["monthly"])
            
        st.number_input(
            f"Monthly Withdrawal ({currency})",
            min_value=0.0, step=1000.0,
            disabled=not editable, key=wd_key,
        )
        scenario["withdrawal"]["monthly"] = st.session_state[wd_key]
        st.caption(f"📅 Yearly Impact: **{currency}{scenario['withdrawal']['monthly'] * 12:,.0f}**")
    ...
    with ui_section("Asset Allocation", "🧮"):
        ...
        def allocation_changed(inst):
            ...
            st.session_state.alloc_state = alloc
            scenario["allocations"] = alloc
            # Force update all sibling inputs so they immediately reflect the math
            for k, v in alloc.items():
                st.session_state[f"alloc_{k}"] = float(v)
        ...
        for i, key in enumerate(alloc_keys):
            ...
            # Setup session state key BEFORE rendering widget (Crucial for sync fix)
            state_key = f"alloc_{key}"
            if state_key not in st.session_state:
                st.session_state[state_key] = float(st.session_state.alloc_state.get(key, 0.0))
        ...
        df_alloc = pd.DataFrame(list(st.session_state.alloc_state.items()), columns=["Instrument", "Allocation"])
        fig_pie = px.pie(df_alloc, values="Allocation", names="Instrument", hole=0.55)
        fig_pie.update_layout(height=280, margin=dict(l=0, r=0, t=10, b=0), showlegend=False)
        fig_pie.update_traces(textposition='inside', textinfo='percent+label')
        st.plotly_chart(fig_pie, use_container_width=True)

        # ---------------------------------------------------------
        # ALLOCATION BREAKDOWN (TABLE & BAR CHART)
        # ---------------------------------------------------------
        st.markdown("### 🧮 Allocation & Income Breakdown")
        
        alloc_rows = []
        for key in alloc_keys:
            label = base_labels.get(key, key)
            alloc_pct = float(scenario["allocations"].get(key, 0))
            if alloc_pct > 0:
                rate = float(scenario["rates"].get(key, 0.0))
                allocated_amt = total_corpus * (alloc_pct / 100)
                annual_income = allocated_amt * (rate / 100)
                
                alloc_rows.append({
                    "Instrument": label,
                    "Alloc (%)": alloc_pct,
                    "Amount": allocated_amt,
                    "Est. Yearly Income": annual_income
                })
        
        if alloc_rows:
            df_breakdown = pd.DataFrame(alloc_rows)
            
            # Format the dataframe cleanly for display
            st.dataframe(
            ...
            # Income Contribution Bar Chart
            fig_income_bar = px.bar(
                df_breakdown, 
                x="Instrument", 
                y="Est. Yearly Income", 
                title="Yearly Income by Instrument",
                text_auto=".2s", 
                color="Instrument"
            )
